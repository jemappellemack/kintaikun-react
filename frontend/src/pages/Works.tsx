...
const Works: FC = () => {
  ...
  const openModal = (date: dateProps) => {
    setSelectDate(date);
    setModalShow(true);
  };
  ...
  return (
    <>
      <Row style={{ marginTop: '2rem' }} className="text-right">
        <Col>
          <Button variant="primary" onClick={tableToExcel}>
            ダウンロード
          </Button>
        </Col>
      </Row>
      <Row style={{ marginTop: '2rem' }} className="text-center">
        <Col>
          {showBackButton ? (
            <Button variant="light" onClick={goBackMonth} block>
              前月へ
            </Button>
          ) : null}
        </Col>
        <Col>
          <h4>{format(targetDate, 'y年M月度')}</h4>
        </Col>
        <Col>
          {showNextButton ? (
            <Button variant="light" onClick={goNextMonth} block>
              翌月へ
            </Button>
          ) : null}
        </Col>
            ...
            <tbody className="text-center">
              {calendars.map((date) => (
                <tr key={date.dateString}>
                  ...
                  <td>
                    {date.week === 'Sa' || date.week === 'Su' ? '休日' : '稼働'}
                  </td>
                  <td>
                    <InjectStoreState
                      store={KintaiCalendarStore}
                      on={(s) => s[date.dateString].startString}>
                  ...
                  <td>
                    <InjectStoreState
                      store={KintaiCalendarStore}
                      on={(s) => s[date.dateString].endString}>
                      {(endString) => <span>{endString}</span>}
                    </InjectStoreState>
                  </td>
                  <td>
                    <InjectStoreState
                      store={KintaiCalendarStore}
                      on={(s) => s[date.dateString].duration}>
                      {(duration) => (
                        <span>
                          {duration.hours}:
